...
- This was a simple implement... if our while loop reaches the end of memory it will automatically print an error message: 'Not enough memory'
...
- In order to handle this case, we have a simple if statement at the start of our free() which compares the user-passed pointer, ptr, to the upper
and lower bounds of our given memory. We compare the address that is ptr with the lower bound, and the upper bound of our memory addresses. This is
...
- Since our previous error detection (checking if the address is outside of the scope of memory) has already been checked... if the while loop finished 
...
memory with our heapStart pointer, we are constantly making comparisons to check if the current or adjacent data block we are on (heapStart + 2 * sizeof(int))
...
- In our if-statement where we check to see if our current data block is equivalent to the user-given ptr, we run a safety check, checking to see if 
the current data block's isFree metadata variable is equivalent to 0 (meaning it is free space in our program). If it is equivalent to 0, then that means
...
1. (a) malloc() reserves unallocated memory [CorrectnessTesting1]
   ...
   (c) This program was provided to us in memtest.c; however, I will explain how it proves (a). This program fills our memory with distinct objects such as '0000' or '1111' when
...
3. (a) malloc() and free() arrange so that adjacent free blocks are coalesced [CorrectnessTesting3]
   ...
   (c) Let us define two pointers and consider the size of memory to be 4096 bytes:
        ...
        - In such case, if coalescense in our program did not occur, a malloc(4000) call would result in and error message: "Not enough memory" as neither of the 2000 byte chunks would be able to hold 4000 bytes
...
4. (a) Errors mentioned in '<><>Reporting Errors<><> are detected and reported [CorrectnessTesting4]
   ...
   (c) For the first error [1].. let's: malloc(4000);
        - Considering the size of our memory in this example is 4096 bytes, this malloc call will work properly
